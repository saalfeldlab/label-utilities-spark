...
public class SparkWatershedsOnDistanceTransform {
	...
	private static class Args implements Serializable, Callable<Void> {
		...
		@Expose
		@CommandLine.Option(names = "--affinity-threshold-distance-transform", paramLabel = "AFFINITY_THRESHOLD_DISTANCE_TRANSFORM", description = "Boundary distance transform will be generated on mask generated by thresholding affinities at this value.", defaultValue = "0.9")
		Double affinityThresholdDistanceTransform;
	...
	public static void run(
		...
		if (relabel) {
			...
			writer.setAttribute(watershedSeeds, "maxId", maxId);
			writer.setAttribute(merged, "maxId", maxId);
			writer.setAttribute(seededWatersheds, "maxId", maxId);
	...
	private static <T extends IntegerType<T> & NativeType<T>> void relabel(
		...
		final String writerCacheKey = new URIBuilder(n5.getURI()).setParameters(
				...
				new BasicNameValuePair("call", "spark-watersheds-distance-transform-relabel")
		).toString();
	...
	private static <UF extends UnionFind> UF findOverlappingLabelsThresholdMedianEdgeAffinities(
		...
		final List<Tuple2<long[], long[]>> doubleSizeBlocks = Grids
				...
				.stream()
				.map(interval -> SparkWatershedsOnDistanceTransform.toMinMaxTuple(interval, Tuple2::new))
				.collect(Collectors.toList());
		...
		final List<Tuple2<long[], long[]>> mappings = sc
				...
				.mapToPair(minMax -> {
					...
					final String labelsCacheKey = new URIBuilder(labelsReader.getURI()).setParameters(
							new BasicNameValuePair("call", "spark-watersheds-distance-transform-find-overlapping-labels"),
	...
	private static <UF extends UnionFind> UF findOverlappingLabelsArgMaxNoHalo(
		...
		final List<Tuple2<long[], long[]>> mappings = sc
				...
				.mapToPair(minMax -> {
					...
					final String labelsCacheKey = new URIBuilder(labelsReader.getURI()).setParameters(
							new BasicNameValuePair("call", "spark-watersheds-distance-transform-find-overlapping-labels-no-halo"),
							new BasicNameValuePair("container", "labels"),
							new BasicNameValuePair("dataset", labelDataset)
					).toString();
	...
	private static class CropAffinitiesToDistanceTransform implements PairFunction<Interval, Interval, Tuple2<RandomAccessibleInterval<FloatType>, RandomAccessibleInterval<DoubleType>>> {
		...
		public Tuple2<Interval, Tuple2<RandomAccessibleInterval<FloatType>, RandomAccessibleInterval<DoubleType>>> call(final Interval interval) throws Exception {
			...
			final String cacheKey = new URIBuilder(n5Local.getURI()).setParameters(
					...
					new BasicNameValuePair("call", "spark-watersheds-distance-transform")
			).toString();
