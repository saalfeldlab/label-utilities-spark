...
public class SparkWatershedsOnDistanceTransformOfSampledFunctionSeedOnlyOnEdge {
	...
	private static <T extends IntegerType<T> & NativeType<T>> boolean relabel(
		...
		final String writerCacheKey = new URIBuilder(n5.getURI()).setParameters(
				...
				new BasicNameValuePair("call", "spark-watersheds-distance-transform-of-sample-edge-only-relabel")
		).toString();
		...
		final List<Tuple2<long[], long[]>> mappings = sc
				...
				.mapToPair(minMax -> {
					...
					final String labelsCacheKey = new URIBuilder(labelsReader.getURI()).setParameters(
							new BasicNameValuePair("call", "spark-watersheds-distance-transform-of-sample-edge-only-find-overlapping-labels"),
	...
	private static <UF extends UnionFind> UF findOverlappingLabelsArgMaxNoHalo(
		...
		final List<Tuple2<long[], long[]>> mappings = sc
				...
				.mapToPair(minMax -> {
					...
					final String labelsCacheKey = new URIBuilder(labelsReader.getURI()).setParameters(
							new BasicNameValuePair("call", "spark-watersheds-distance-transform-of-sample-edge-only-find-overlapping-labels-no-halo"),
							new BasicNameValuePair("container", "labels"),
							new BasicNameValuePair("dataset", labelDataset)
					).toString();
